...
async def send_webhook_alert(webhook_url: str, alert_payload: dict):
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            logger.info(f"[BACKGROUN TASK FINISHED] webhook delivered! status: {response.status_code}")
    except Exception as e:
        logger.error(f"[BACKGROUND TASK FAILDE] Webhook delivered failed: {str(e)}") 
